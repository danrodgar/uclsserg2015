...
## The Aim of Data Analysis and Statistical Learning
  ...
  - We want to generate conclusions about the population where our data comes from
...
## Normality 
  ...
  - Two examples: one population with mean 60 and the standard deviation of 1, and the other with mean 60 and    (means shifted to 0)
...
## Normality. Galton data
  - It is the data based on the famous 1885 study of Francis Galton exploring the relationship between the heights of adult children and the heights of their parents.
...
## Measures of Evaluation used in Software Engineering
  ...
  - *Mean of the Absolute Error*: compute the absolute errors and take the mean
  - *Geometric Mean of the Absolute Error*: more appropriate when the distribution is skewed
  ...
  - *Level of Prediction (LPred(l))*: defined as the percentage of estimates that are within the level l% of the actual values. Usually the level of prediction is set at 25% below and above the actual value and an estimation method is considered good if give a result of more than 75%.
  - *Standardised Accuracy (SA)*: this measure overcomes all the problems of the MMRE (proposed by Shepperd&MacDonnell). It is defined as the MAR relative to random guessing:
$$
...
```
  ...
  - ‘95%’ confidence interval means: is the probability that the hypothetical confidence intervals (that would be computed from the hypothetical repeated samples) will contain the population mean.
